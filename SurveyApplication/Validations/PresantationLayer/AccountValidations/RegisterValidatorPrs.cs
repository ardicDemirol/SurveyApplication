...
public class RegisterValidatorPrs : AbstractValidator<RegisterCommandRequest>
{
    public RegisterValidatorPrs()
    {
        ...
        RuleFor(request => request.role)
            .NotEmpty().WithMessage("{PropertyName} must not be empty.")
            .Must(role => role == "admin" || role == "user").WithMessage("{PropertyName} must be either 'admin' or 'user'.");
    }
}
